...
| `tabs` | `tabs` | Viisi välilehteä; Info sisältää katkohistorian; profiili ja turnaus porautumisina; viesti profiililta tallentuu | `DgTabs.kt`, `InfoScreen.kt` (`InfoSection.Drops`), `PageScreen.kt`, `page_message_sent` |
...
| `board` | `unconfirmed` | Board not confirmed, asema on viimeinen ladattu; jokainen katko Connection drops -listaan Info-välilehdelle (hetki, ottelu, nappi, virheen nimi), sata viimeistä | `board_unconfirmed_*`, `BoardViewModel.afterPress`, `DropLog`, `DropsScreen.kt` |
...
| `board` | `look` | Kolme lautatyyliä ja värit, variantin paneeli lokeron puuta; noppa- ja pistetyyli; odotuksen ilmaisin (kaari, ouroboros, kuutio); robotti ja CC BY | `BoardLook.kt` (`PanelLook`), `BoardStyleStore`, `DiceStyleStore`, `ScoreStyleStore`, `BusyStyleStore`, `BusyIndicator`, `DgBoard.kt` |
